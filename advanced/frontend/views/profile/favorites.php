...
<div class="container mx-auto px-4 py-12 min-h-[60vh]">
    ...
    <div class="mb-12 text-center animate-fade-in-down">
        ...
        <p class="text-gray-500 text-lg">
            <?php if ($count > 0): ?>
                Você tem <strong><?= $count ?></strong> <?= $count === 1 ? 'produto favorito' : 'produtos favoritos' ?>
            <?php else: ?>
                ...
                <div class="bg-white rounded-2xl overflow-hidden hover:shadow-2xl hover:shadow-pink-100/50 transition-all duration-300 border border-gray-100 group flex flex-col relative">
                    ...
                    <div class="relative overflow-hidden aspect-[3/4]">
                        ...
                        <?php if (isset($product['old_price']) && $product['price'] < $product['old_price']): ?>
                            <span class="absolute top-3 left-3 bg-rose-500 text-white text-xs font-bold px-2 py-1 rounded shadow-md z-10">
                                PROMOÇÃO
                            </span>
                        <?php endif; ?>
                        ...
                        <a href="<?= Url::to(['site/remove-favorite', 'id' => $product['id']]) ?>"
                           class="absolute top-3 right-3 w-10 h-10 bg-white/90 backdrop-blur-sm rounded-full shadow-md text-gray-400 hover:text-white flex items-center justify-center hover:bg-red-500 transition-all transform hover:rotate-90 z-20"
                    ...
                    <div class="p-5 flex-1 flex flex-col">
                        <div class="flex-1">
                            <a href="<?= Url::to(['site/product', 'id' => $product['id']]) ?>" class="block group-hover:text-rose-600 transition-colors">
                        ...
                        <div class="flex items-center justify-between mt-4 pt-4 border-t border-gray-50">
                            ...
                            <button onclick="addToCart('<?= Html::encode($product['name']) ?>')" class="w-10 h-10 rounded-full bg-gray-100 text-gray-800 hover:bg-rose-500 hover:text-white transition-all flex items-center justify-center shadow-sm hover:shadow-lg" title="Adicionar ao Carrinho">
